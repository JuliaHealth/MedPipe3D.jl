...
@testset "io_utils" begin
	...
	@testset "load_images_from_hdf5" begin
		h5path = _write_v2_hdf5(joinpath(tmp, "test_load.h5");
			batch_size = 2, channels = 2, sz = (8, 8, 8))
		img_batches, img_meta, mask_batches, mask_meta =
			load_images_from_hdf5(h5path)
	...
	@testset "get_class_or_split_from_json" begin
		...
		@test get_class_or_split_from_json(
			"/data/patient_01/images/scan.nii", json_path) == "tumour"
		@test get_class_or_split_from_json(
			"/data/patient_02/images/scan.nii", json_path) == "healthy"

		# No match → nothing
		@test get_class_or_split_from_json(
			"/data/patient_99/images/scan.nii", json_path) === nothing
