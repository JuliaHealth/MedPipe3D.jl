...
function get_class_labels(patient_groups::Vector, h5::HDF5.File, config::Dict)
    ...
    for patient_group_name in patient_groups
        ...
        if group_exists(h5, metadata_images_path)
            ...
            class_label = if get(config["learning"], "class_JSON_path", false) != false
                safe_read_attribute(meta_subgroup, "class")
            else
                "class1"
            end
            found = true
...
function get_class_mapping(patient_groups, h5, config)
    class_mapping = Dict{String, Vector{String}}()  # String keys, vector of group names
    class_counter = Dict{String, Int}()             # Index per unique class name

    for group in patient_groups
        class_dict = get_class_labels([group], h5, config)
        class_name = only(keys(class_dict))

        isa(class_name, String) ||
            error("Expected class_name to be a String, got $(typeof(class_name))")

        if !haskey(class_counter, class_name)
            class_counter[class_name] = length(class_counter) + 1
        end
        unique_class_key = "$(class_counter[class_name])_$class_name"
