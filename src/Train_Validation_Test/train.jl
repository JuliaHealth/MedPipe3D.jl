...
# Description
Updates the best metric if current performance is better; increments the patience counter
otherwise. Stops training if improvements are not observed for a configured number of epochs.
Supports both the new nested schema (`config["model"]["early_stopping"]["min_delta"]`) and
the flat legacy schema (`config["model"]["early_stopping_min_delta"]`).
"""
function check_early_stopping(current_metric, best_metric, config, counter)
	# Support both new nested schema and flat legacy schema
	_es_cfg = get(config["model"], "early_stopping", Dict())
	early_stopping_min_delta = isa(_es_cfg, Dict) ?
							   get(_es_cfg, "min_delta", 0.001) :
							   get(config["model"], "early_stopping_min_delta", 0.001)
	_patience = isa(_es_cfg, Dict) ?
				get(_es_cfg, "patience", 5) :
				get(config["model"], "patience", 5)
...
"""
`train_epoch(train_group_paths, validation_group_paths, h5, model, tstate, config,
			 loss_function, num_classes, early_stopping_dict = nothing)`
...
# Description
Performs training operations by processing batches of data, evaluating performance, and
applying the optimizer. Optionally evaluates early stopping criteria based on validation
loss or another configured metric. Supports both the new nested early_stopping schema and
the flat legacy schema.
"""
function train_epoch(tain_group_paths, validation_group_paths, h5, model, tstate, config, loss_function, num_classes, early_stopping_dict = nothing)
	...
	# Unwrap early_stopping — new schema stores it as a nested dict with "enabled" key;
	# old schema stores a plain bool. Both are supported.
	...
	for batch_idx in 1:num_batches
		...
		println("Batch $batch_idx, Training loss: $loss")

		if early_stopping
			mean_val_metric, mean_val_loss = evaluate_validation(
				validation_group_paths, h5, model, tstate, loss_function, config, num_classes,
			)
			y_pred, _ = infer_model(tstate, model, data)
			...
			best_metric, patience_counter, stop_training = check_early_stopping(
				current_val_metric, best_metric, config, patience_counter,
			)
			if stop_training
				println("Metric $early_stopping_metric last best value: $best_metric after $patience_counter batches with improvement less than $early_stopping_min_delta.")
				break
			end
		end
	end
